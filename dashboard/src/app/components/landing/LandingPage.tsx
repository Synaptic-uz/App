...
import { LandingChatMockup } from './LandingChatMockup';
...
export function LandingPage() {
  ...
  return (
    <div className="bg-[var(--color-bg)] text-[var(--color-text)]">
        ...
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)]">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-5">
            {PROBLEMS.map(({ icon: Icon, title, desc }) => (
              <div
                key={title}
                className="rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-xs)]"
              >
        ...
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)]">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-5">
            {PILLARS.map(({ icon: Icon, title, desc }) => (
              <div
                key={title}
                className="rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6 hover:border-[var(--color-primary)]/25 hover:shadow-[var(--shadow-sm)] transition-all"
              >
      ...
      {/* Pipeline */}
      <section className="py-16 md:py-20 bg-[#0f1419] text-white">
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)]">
          <SectionLabel>
          ...
          <ol className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {PIPELINE.map((step) => (
              <li
                key={step.n}
                className="rounded-[var(--radius-lg)] border border-white/10 bg-white/5 p-5 backdrop-blur-sm"
              >
            ...
            <div>
              ...
              <div className="mt-8 space-y-4">
                {MARKET.map((m) => (
                  <div
                    key={m.label}
                    className="flex items-center justify-between gap-4 rounded-[var(--radius-lg)] border border-[var(--color-border)] px-5 py-4"
                  >
                    <div>
              ...
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-bg-subtle)]">
                  {COMPARE[0].map((cell, i) => (
                    <th
                      key={i}
                      className={`py-3 px-4 text-left font-semibold ${i === 3 ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}
                    >
                      {cell}
                    </th>
                  ))}
                ...
                {COMPARE.slice(1).map((row) => (
                  <tr key={row[0]} className="border-b border-[var(--color-border)] last:border-0">
                    {row.map((cell, i) => (
                      <td
                        key={i}
                        className={`py-3 px-4 ${i === 0 ? 'font-medium' : ''} ${i === 3 ? 'text-[var(--color-primary)] font-semibold' : 'text-[var(--color-text-secondary)]'}`}
                      >
                        {cell}
                      </td>
                    ))}
        ...
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)]">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-5">
            {ROADMAP.map((r) => (
              <div
                key={r.phase}
                className="rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6 flex flex-col"
              >
      ...
      <section className="py-16 md:py-20 border-t border-[var(--color-border)]">
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)]">
          ...
          <p className="text-center text-[var(--color-text-secondary)] mt-3 max-w-xl mx-auto">
          ...
          <div className="mt-10 grid sm:grid-cols-2 gap-5 max-w-3xl mx-auto">
            ...
            >
              ...
              <ul className="mt-3 space-y-2 text-sm text-[var(--color-text-secondary)]">
                {[
                  t('landing.audience.business.features.native'),
                  t('landing.audience.business.features.wallet'),
                  t('landing.audience.business.features.analytics')
                ].map((text) => (
            ...
            >
              ...
              <ul className="mt-3 space-y-2 text-sm text-[var(--color-text-secondary)]">
                {[
                  t('landing.audience.agent.features.api'),
                  t('landing.audience.agent.features.revShare'),
                  t('landing.audience.agent.features.demo')
                ].map((text) => (
                  <li key={text} className="flex items-center gap-2">
                    <Check className="w-4 h-4 text-[var(--color-primary)] shrink-0" />
                    {text}
      ...
      {/* Final CTA */}
      <section className="py-16 md:py-20 bg-gradient-to-br from-[var(--color-primary)] to-[#0026e6] text-white">
        <div className="mx-auto max-w-[var(--page-max)] px-[var(--page-px)] text-center">
          <Logo size="lg" className="mx-auto mb-6 ring-2 ring-white/30" />
          <h2 className="text-2xl md:text-4xl font-extrabold tracking-tight max-w-2xl mx-auto">
            {t('landing.final.title')}
          </h2>
          <p className="mt-4 text-white/85 max-w-lg mx-auto leading-relaxed">
