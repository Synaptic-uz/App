...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || uz.auth.registerFailed);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        <div className="text-center">
          <h2 className="text-3xl font-extrabold text-gray-900">{uz.auth.createTitle}</h2>
          <p className="mt-2 text-sm text-gray-600">{uz.auth.createSubtitle}</p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-4 mb-6">
            ...
            >
              ...
              <span className={`text-sm font-medium ${role === 'business' ? 'text-gray-900' : 'text-gray-500'}`}>{uz.auth.business}</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span className={`text-sm font-medium ${role === 'agent' ? 'text-gray-900' : 'text-gray-500'}`}>{uz.auth.aiAgent}</span>
            </button>
          </div>
          ...
          <div>
            ...
            >
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : uz.common.createAccount}
            </button>
          </div>

          <div className="text-center text-sm">
            <span className="text-gray-600">{uz.auth.hasAccount} </span>
