...
export async function loadCampaignCache() {
  ...
  const campaigns = await Campaign.find({ active: { $in: [1, true] } })
    .select("+embedding")
    .lean();
  const { hasCampaignBudgetRemaining } = await import("./wallet.js");
  campaignCache = campaigns.filter(hasCampaignBudgetRemaining).map((c) => ({
    _id: c._id,
    name: c.name,
    category: c.category,
...
function getCachedResponse(prompt, messages, answerOnly = false) {
  const key = historyCacheKey(prompt, messages, answerOnly);
...
function setCachedResponse(prompt, messages, data, answerOnly = false) {
  if (responseCache.size >= MAX_CACHE_SIZE) {
    const oldestKey = responseCache.keys().next().value;
    responseCache.delete(oldestKey);
  }
  responseCache.set(historyCacheKey(prompt, messages, answerOnly), {
    data,
    timestamp: Date.now(),
  });
}
...
function ruleBasedIntent(prompt) {
  ...
  if (hotScore >= 0.6)
    return { intent: "hot", confidence: Math.min(hotScore, 1), method: "rule" };
  if (warmScore >= 0.6)
    return {
      intent: "warm",
      confidence: Math.min(warmScore, 1),
      method: "rule",
    };
  if (hotScore >= 0.3)
    return { intent: "warm", confidence: hotScore, method: "rule" };
...
export async function enrichResponse(prompt, sessionId = null, options = {}) {
  const { answerOnly = false, messages = [], parseMode = "Markdown" } = options;
  const startTime = Date.now();
  const session = sessionId || nanoid(16);
  const history = normalizeChatHistory(messages);
  const matchPrompt = buildMatchingPrompt(prompt, history);
  const useMarkdown = parseMode === "Markdown" || parseMode === "markdown";

  const cached = getCachedResponse(prompt, history, answerOnly);
  if (cached && history.length <= 2) {
    console.log(`Response cache HIT (${Date.now() - startTime}ms)`);
    return { ...cached, session_id: session, _cached: true };
  }

  const ruleIntent = ruleBasedIntent(matchPrompt);
  let intentResult;
  ...
  if (ruleIntent) {
    ...
    console.log(
      `Intent via rules: ${ruleIntent.intent} (${(ruleIntent.confidence * 100).toFixed(0)}%)`,
    );
  } else {
    console.log("Classifying intent via LLM...");
    try {
      intentResult = await classifyIntent(matchPrompt);
    } catch (err) {
  ...
  if (answerOnly) {
    try {
      const result = await generateStandardContent(
        prompt,
        intentResult,
        history,
        useMarkdown,
    ...
    try {
      ...
      if (bestCampaign) {
        console.log(
          `Matched: ${bestCampaign.name} (${match.method}, score: ${highestScore.toFixed(3)}, topic: ${match.dominantTopic || "n/a"})`,
        );
      } else if (match.dominantTopic) {
        console.log(
          `No campaign for detected topic "${match.dominantTopic}" — skipping ad`,
        );
      }
    } catch (err) {
    ...
    if (bestCampaign) {
      ...
      const generator =
        generators[intentResult.intent] || generateColdIntentResponse;
      console.log(
        `${intentResult.intent.toUpperCase()} intent → ${bestCampaign.name} (${highestScore.toFixed(3)})`,
      );
      ...
      console.log(
        `No match (score: ${highestScore > -1 ? highestScore.toFixed(3) : "N/A"})`,
      );
      try {
        const result = await generateStandardContent(
          prompt,
  ...
  const result = {
    ...
    usage: {
      ...
      generation: genUsage,
      total_tokens:
        (intentResult.usage?.total_tokens || 0) + (genUsage?.total_tokens || 0),
    },
    performance: {
  ...
  if (
    (answerOnly || !appliedCampaign || intentResult.intent === "cold") &&
    history.length <= 2
  ) {
    setCachedResponse(prompt, history, result, answerOnly);
  }
...
function buildLlmMessages(
  ...
  const lastIsCurrent =
    ...
    turns[turns.length - 1].content === String(latestUserPrompt || "").trim();

  const prior = lastIsCurrent ? turns.slice(0, -1) : turns;
  for (const t of prior) {
    msgs.push({ role: t.role, content: t.content });
  }

  msgs.push({ role: "user", content: String(latestUserPrompt || "").trim() });
  return msgs;
}
...
// --- Response Generators ---
async function generateHotIntentResponse(
  prompt,
  campaign,
  intent,
  history = [],
...
RULES: 1) Direct answer first 2) Natural recommendation 3) Mention deals/muddatli to'lov 4) NEVER paste URLs — only mention brand by name: ${campaign.link_text || campaign.name} 5) Concise 6) Match language. 7) Use prior messages for context.
BRAND: ${campaign.name} | ${campaign.category} | ${campaign.description}`;
  ...
  const response = await getClient()
    ...
    .post({
      body: {
        messages: buildLlmMessages(systemPrompt, history, prompt, useMarkdown),
        model: modelName,
        temperature: 0.3,
      },
    });
...
async function generateWarmIntentResponse(
  prompt,
  campaign,
  intent,
  history = [],
...
RULES: 1) Comparative info 2) Educational 3) Campaign as one option 4) NEVER paste URLs — brand name only: ${campaign.link_text || campaign.name} 5) Match language. 6) Use chat history.
BRAND: ${campaign.name} | ${campaign.category} | ${campaign.description}`;
...
async function generateColdIntentResponse(
  prompt,
  campaign,
  intent,
  history = [],
...
RULES: 1) Answer fully first 2) Mention brand only if relevant 3) NEVER paste URLs — brand name only: ${campaign.link_text || campaign.name} 4) Match language. 5) Use chat history.
BRAND (if relevant): ${campaign.name} | ${campaign.description}`;
...
async function generateStandardContent(
  prompt,
  intent,
  history = [],
  useMarkdown = true,
) {
  const systemPrompt = `Synaptic AI assistant. Intent: ${intent.intent.toUpperCase()}.
Answer the user's question directly and completely using the full conversation context.
Match their language (Uzbek/Russian/English).
Do NOT mention sponsors, ads, brands, stores, or product links.
For informational questions give facts only — never pivot to unrelated shopping.`;
